...
| prj-gha-cache-poison | Cache Key Not Bound to a Lockfile Hash | WARN | MEDIUM | 5.0 | A `.github/workflows/*` `actions/cache` step keys its cache with no integrity binding — neither a `hashFiles()` over a lockfile nor a commit-SHA context (CWE-494). A cache poisoned from a lower-trust context (fork PR, feature branch) persists into deploy-preview builds. |
| prj-gha-cache-restore-keys | Broad restore-keys Fallback Bypasses Cache Key Hash | WARN | LOW | 3.0 | The cache `key` is integrity-bound but `restore-keys` uses an unbound prefix, so a key miss falls back to any older entry regardless of the current lockfile — reopening the poisoning path. |

> Project mode also emits `PASS` findings for clean files, an informational `prj-next-cve` (WARN) when the `next` version is a range/tag that cannot be pinned for CVE evaluation, `prj-scope` (PASS) when no recognizable project files are found, and `ERROR` findings (e.g. `prj-vercel-json`, `prj-nextconfig`, `prj-gha-cache-poison`) when a present file is unreadable or unparseable.
